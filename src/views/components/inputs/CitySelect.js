...
function CitySelect(props) {
  ...
  const fetchZipcodes = React.useCallback(async () => {
    try {
      let result = await addressRepository.getCities({
      ...
      if (result !== undefined) {
        ...
        if (result?.data?.length > 0) {
          result.data.map((data, index) => {
            init.push({ id: index, value: data.area, zipcode: data.zipcode });
          });
        }
        setZipcodes(init);
      }
  ...
  return (
    <React.Fragment>
      <SelectDropdown
        ref={ref}
        search
        searchPlaceHolder="Type the city"
        data={zipcodes}
        disabled={props.editable ? props.editable : false}
        onSelect={(selectedItem, _) => {
          handleSelected(selectedItem.zipcode);
          if (props.setCity) {
            props.setCity(selectedItem.value);
          }
        }}
        ...
        defaultButtonText={props.values.city ? props.values.city : "City *"}
      ...
      {props.errors.zipcode && props.touched.zipcode && (
        <HelperText type="error" visible={props.errors.zipcode}>
